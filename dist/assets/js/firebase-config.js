...
function getEnvVar(key) {
    // Check if environment loader is available
    if (window.ENV && window.ENV.get) {
        return window.ENV.get(key);
    }
    
    // Fallback for direct environment access (if available)
    if (typeof process !== 'undefined' && process.env) {
        return process.env[key];
    }
    
    // Development fallback - in production these should be injected at build time
